    ...
    <div className="space-y-4">
      ...
      {task && (
        <div className={`rounded-lg p-4 shadow-lg ${
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {task.completed && (
              <span className="bg-green-500 text-white px-2 py-1 rounded text-xs">已完成</span>
            )}
          </div>
          <p className="text-gray-700 mb-2">{task.description}</p>
          <p className="text-sm text-gray-600 mb-2">目标：{task.target}</p>
          {task.remainingTurns !== undefined && !task.completed && (
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">剩余回合</span>
      ...
      {threat && (
        <div className="bg-red-50 border-2 border-red-400 rounded-lg p-4 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg font-bold text-red-800">⚠️ 威胁事件</h3>
          ...
          <p className="font-semibold text-red-700 mb-1">{threat.name}</p>
          <p className="text-gray-700 mb-2">{threat.description}</p>
          <p className="text-sm text-gray-600 mb-2">要求：{threat.requirement}</p>
      ...
      {!task && !threat && (
        <div className="bg-white rounded-lg p-4 shadow-lg text-center text-gray-500">
          <p>暂无任务或威胁</p>
        </div>
      )}
    </div>
